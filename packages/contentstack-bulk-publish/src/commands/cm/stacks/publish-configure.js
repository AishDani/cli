  ...
  async run() {
    ...
    if (configureFlags.alias) {
      try {
        ...
        this.error(
          `The configured @contentstack/management token alias ${configureFlags.alias} has not been added yet. Add it using 'csdx auth:tokens:add -a ${configureFlags.alias}'`,
          { exit: 2 },
        );
      }
