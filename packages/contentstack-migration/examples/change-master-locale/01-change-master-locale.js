        ...
        for (let contentType of contentTypes) {
          sourceMasterLocaleEntries = await fs.readFile(
            pathValidator(
              path.resolve(
                ...
                `entries/${sanitizePath(contentType)}/${sanitizePath(masterLocale)}.json`,
              ),
            ),
            { encoding: 'utf8' },
          );
          ...
          targetMasterLocaleEntries = await fs.readFile(
            pathValidator(
              path.resolve(
                sanitizePath(config.data_dir),
                `entries/${sanitizePath(contentType)}/${sanitizePath(config.target_locale)}.json`,
              ),
            ),
          ...
          await fs.writeFile(
            pathValidator(path.resolve(config.data_dir, `entries/${contentType}/${config.target_locale}.json`)),
